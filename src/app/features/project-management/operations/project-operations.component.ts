...
export class ProjectOperationsComponent implements OnInit {
  ...
  get isSuperAdmin(): boolean {
    return (
      (this.auth.getCurrentUser()?.role || "").toLowerCase() === "superadmin"
    );
  }
  ...
  get filteredDeliveries(): PmEmailLog[] {
    ...
    return this.deliveries.filter(
      (x) =>
        !q ||
        `${x.entityType} ${x.emailType} ${x.recipientEmail} ${x.deliveryStatus} ${x.subject}`
          .toLowerCase()
          .includes(q),
    );
  }
  ...
  load(): void {
    this.api.settings().subscribe({
      next: (v) =>
        v.forEach((x) => {
          const k =
            x.settingKey.charAt(0).toLowerCase() + x.settingKey.slice(1);
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (e) =>
          (this.error =
            e?.error?.message || "Unable to load email deliveries."),
      });
    this.api
      .customerResponses()
      .subscribe({ next: (v) => (this.responses = v) });
    this.api.enquiries().subscribe((v) => (this.enquiries = v));
    this.api.quotations().subscribe((v) => (this.quotations = v));
    this.api.projects().subscribe((v) => (this.projects = v));
  }
  saveSettings(): void {
    this.clear();
    this.api
      ...
      .subscribe({
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to save settings."),
      });
  }
  ...
  saveResponse(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to record response."),
      });
  }
