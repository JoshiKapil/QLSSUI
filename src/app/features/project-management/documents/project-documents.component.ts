...
export class ProjectDocumentsComponent implements OnInit {
  ...
  get filteredProjects(): PmProject[] {
    ...
    return this.projects.filter(
      (p) => !q || `${p.projectNo} ${p.customerName} ${p.projectTitle}`.toLowerCase().includes(q),
    );
  }
  select(p: PmProject): void {
    this.selected = p;
    this.api
      .attachments('Project', p.projectId)
      .subscribe({ next: (v) => (this.files = v), error: () => (this.files = []) });
  }
  upload(event: Event): void {
    ...
    this.api.uploadAttachment('Project', this.selected.projectId, file).subscribe({
      ...
      error: (e) => (this.error = e?.error?.message || 'Unable to upload document.'),
    });
  }
  download(f: PmAttachment): void {
    this.api.attachmentFile(f.attachmentId).subscribe((blob) => {
  ...
  icon(f: PmAttachment): string {
    const n = f.originalFileName.toLowerCase();
    if (n.endsWith('.pdf')) return 'fa-file-pdf';
    if (n.endsWith('.xlsx') || n.endsWith('.xls')) return 'fa-file-excel';
    if (n.endsWith('.docx') || n.endsWith('.doc')) return 'fa-file-word';
    if (/\.(png|jpg|jpeg|webp)$/.test(n)) return 'fa-file-image';
