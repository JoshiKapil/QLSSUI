...
export class ProjectApprovalsComponent implements OnInit {
  ...
  decideQuotation(q: PmQuotation, action: 'Approve' | 'Reject' | 'Return'): void {
    const remark = action === 'Approve' ? '' : (prompt(`${action} remark:`, '') || '').trim();
    if (action !== 'Approve' && !remark) return;
    this.api.decideQuotation(q.quotationId, action, remark, false, Math.max(1, +this.followUpDays || 2)).subscribe({
      next: () => {
        this.success = `${q.quotationNo}: ${action} completed.`;
        this.load();
      },
  ...
  decideClosure(p: PmProject, action: 'Approve' | 'Return'): void {
    const remark = action === 'Return' ? (prompt('Pending activity / return remark:', '') || '').trim() : '';
    if (action === 'Return' && !remark) return;
    this.api.decideClosure(p.projectId, action, remark, true).subscribe({
      next: () => {
        this.success = `${p.projectNo}: closure ${action.toLowerCase()} processed.`;
        this.load();
      },
