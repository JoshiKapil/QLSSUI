...
export class ProjectQuotationsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.settings().subscribe({
      next: (values) => {
        const item = values.find(
          (x) => x.settingKey === "QuotationFollowUpDays",
        );
    ...
    this.refresh();
    this.api.lookups().subscribe({ next: (v) => {
      this.lookups = v;
  ...
  get filteredItems(): PmQuotation[] {
    ...
    const filtered = this.items.filter(
      (x) =>
        ...
        (!q ||
          `${x.quotationNo} ${x.customerName} ${x.serviceType} ${x.status}`
            .toLowerCase()
            .includes(q)),
    );
    ...
    const selectedIndex = filtered.findIndex(
      (x) => x.quotationId === this.selected?.quotationId,
    );
  ...
  get nextActionTitle(): string {
    const status = this.selected?.status || "";
    if (status === "Approval Pending") return this.isSuperAdmin ? "Review and approve this quotation" : "Waiting for SuperAdmin approval";
    if (status === "Quotation Approved") return "Preview the branded PDF, then send it to the customer";
    if (status === "Quotation Sent") return this.isSuperAdmin ? "Create the project when the customer confirms" : "Record the customer response";
  ...
  get nextActionHelp(): string {
    const status = this.selected?.status || "";
    if (status === "Approval Pending") return this.isSuperAdmin ? "Check the scope and amount, then choose Approve, Return, or Reject below." : "No action is needed now. You can follow the decision in the approval trail.";
    if (status === "Quotation Approved") return "The branded PDF uses the QLSS template and the live Client Master list. Preview it before sending.";

    if (status === "Quotation Sent") return this.isSuperAdmin ? "Open Confirm project, add the delivery team and dates, then create the project." : "Use the customer-response buttons below or ask SuperAdmin to create the confirmed project.";
  ...
  get hasUnsavedPriceChanges(): boolean {
    ...
    return (+this.form.professionalFees || 0) !== (+this.selected.professionalFees || 0)
  ...
  refresh(): void {
    ...
    this.api.quotations().subscribe({
      ...
      error: (e) =>
        (this.error = e?.error?.message || "Unable to load quotations."),
    });
  }
  ...
  async confirmSubmit(): Promise<void> {
    ...
    try {
      ...
      this.api.submitQuotation(q.quotationId).subscribe({
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to submit quotation."),
      });
    } catch (e: any) {
  ...
  decide(q: PmQuotation, action: "Approve" | "Reject" | "Return"): void {
    const remark =
      action === "Approve"
        ? ""
        : (window.prompt(`${action} remark:`, "") || "").trim();
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (e) =>
          (this.error =
            e?.error?.message || "Unable to save quotation decision."),
      });
  }
  ...
  async send(q: PmQuotation): Promise<void> {
    ...
    try {
      ...
      this.api
        .sendQuotation(
          q.quotationId,
          Math.max(1, +this.sendForm.followUpDays || 2),
          this.sendForm.cc || "",
        )
        .subscribe({
          next: () => {
          ...
          error: (e) =>
            (this.error = e?.error?.message || "Unable to send quotation."),
        });
    } catch (e: any) {
  ...
  private async prepareBrandedPdf(q: PmQuotation, download: boolean): Promise<void> {
    ...
    try {
      const canUseForm = this.selected?.quotationId === q.quotationId && ["Draft Quotation", "Quotation Under Preparation", "Returned for Modification"].includes(q.status);
  ...
  scheduleFollowUp(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to schedule follow-up."),
      });
  }
  ...
  sendFollowUp(item?: PmFollowUp): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to send follow-up."),
      });
  }

  completeFollowUp(item: PmFollowUp): void {
    this.api
      .completeFollowUp(item.followUpId, this.followUpForm.notes)
      .subscribe({
        next: () => {
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to complete follow-up."),
      });
  }
  ...
  setOutcome(status: string): void {
    ...
    this.api
      .updateEnquiryStatus(this.selected.enquiryId, status, remark)
      .subscribe({
        next: () => {
        ...
        error: (e) =>
          (this.error =
            e?.error?.message || "Unable to update enquiry outcome."),
      });
  }
  ...
  toggleMember(userId: number, checked: boolean): void {
    ...
    if (!checked)
      this.projectForm.memberUserIds = ids.filter((x) => x !== userId);
  ...
  confirmProject(q: PmQuotation): void {
    ...
    const leaderId = this.projectForm.projectLeaderUserId
      ? +this.projectForm.projectLeaderUserId
      : null;
    const memberIds = [
      ...new Set<number>([
        ...(this.projectForm.memberUserIds || []),
        ...(leaderId ? [leaderId] : []),
      ]),
    ];
    this.clearMessage();
    this.api
      ...
      .subscribe({
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to confirm project."),
      });
  }
  ...
  upload(event: Event): void {
    ...
    if (!file) return;
    this.api
      .uploadAttachment("Quotation", this.selected.quotationId, file)
      .subscribe({
        next: () => {
        ...
        error: (e) =>
          (this.error = e?.error?.message || "Unable to upload attachment."),
      });
  }
  ...
  badge(status: string): string {
    ...
    if (s.includes("approved") || s.includes("sent"))
  ...
  private ensureDefaultProjectTemplate(): void {
    ...
    const quotationCategoryId = this.selected.categoryId
      || this.lookups?.categories.find(category =>
        category.name.trim().toLowerCase() === this.selected?.serviceType.trim().toLowerCase()
      )?.id;
    if (!quotationCategoryId) return;
    const match = this.projectTemplates.find(template =>
      template.isActive && +template.categoryId! === +quotationCategoryId
    );
    if (match) {
