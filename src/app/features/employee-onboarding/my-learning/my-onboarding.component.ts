...
@Component({
  ...
  templateUrl: './my-onboarding.component.html',
  styleUrls: ['./my-onboarding.component.scss'],
})
export class MyOnboardingComponent implements OnInit, OnDestroy {
  readonly notificationsPage = new ListPage('Notifications');
  reloadnotificationsPage(): void { this.loadNotifications(); }
  ...
  load(preferredItemId?: number): void {
    ...
    this.api.my().subscribe({
      next: (data) => {
        ...
        const current =
          preferred ||
          data.items.find((x) => ['Available', 'VideoInProgress', 'AssignmentReady'].includes(x.status)) ||
      ...
      error: () => {
        this.loading = false;
        this.notifier.warningToastr('Unable to load employee onboarding.');
      },
    });
  }
  ...
  select(item?: OnboardingEmployeeItem, saveFirst = true): void {
    ...
    if (saveFirst && this.selected && this.selected.enrollmentItemId !== item.enrollmentItemId)
      this.saveCurrentProgress(false);
    this.selected = item;
    this.answers = {};
    ...
    this.api.submitAssignment(this.selected.enrollmentItemId, answers).subscribe({
      next: (result) => {
        ...
        const message = showScore
          ? result.passed
            ? `Assignment completed. Score ${result.percentage}%`
            : `Score ${result.percentage}%. Please try again.`
          : result.passed
            ? 'Assignment completed successfully.'
            : 'Assignment submitted. Please try again.';
      ...
      error: (err) => {
        this.submitting = false;
        this.notifier.warningToastr(err?.error?.message || 'Assignment could not be submitted.');
      },
    });
  }
  ...
  markRead(notification: OnboardingNotification): void {
    ...
    this.api.markNotificationRead(notification.notificationId).subscribe(() => {
      ...
      this.data.unreadNotifications = Math.max(0, this.data.unreadNotifications - 1);
    });
  }

  private loadNotifications(): void {
    this.api.notifications(this.notificationsPage).subscribe((rows) => (this.notifications = rows));
  }
  ...
  private saveCurrentProgress(ended: boolean): void {
    ...
    this.api
      .saveVideoProgress(item.enrollmentItemId, Math.floor(video.currentTime), Math.floor(video.duration), ended)
      .subscribe({
        next: (updated) => {
          ...
          if (updated.status === 'AssignmentReady')
            this.notifier.successToastr('Video completed. Your assignment is now unlocked.');
        },
        error: () => {
          this.savingProgress = false;
        },
